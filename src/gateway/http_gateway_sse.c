 ...
 * @brief SSE tool-loop content-reader state machine and route handler for
 *        the HTTP gateway streaming subsystem.
 *
 * Originally part of http_gateway_routes.c (2300+ lines); extracted into
 * its own TU and later split across cohesive modules (shared types and
 * cross-module declarations live in http_gateway_sse_internal.h, private
 * to the SSE subsystem):
 *
 *   http_gateway_sse.c        — this file: MHD content_reader state machine
 *                               (LLM_ROUND/LLM_STREAM/EXEC_TOOLS/REASONING/
 *                               FINAL_TEXT), socket recv, route handler
 *   gateway_sse_frame.c       — SSE event → wire-frame pure encoders
...
/* ── SSE frame emitters ──────────────────────────────────────────────
 * gw_sse_frame / gw_sse_emit_{reasoning,usage,error,keepalive,tool_call,
 * tool_result} now live in gateway_sse_frame.c (pure wire encoders).
 * ─────────────────────────────────────────────────────────────────── */
                           ...
                           http_request_context_t *context)
{
     ...
     *（/api/v1/run）；下方 legacy 编排体留作可追溯死代码，随 0.2.x 网关
